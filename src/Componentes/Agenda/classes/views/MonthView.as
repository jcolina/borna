package Componentes.Agenda.classes.views
{
	...
	import Componentes.Agenda.mxml_views.monthCell;
	...
	public class MonthView extends Canvas
	{
		...
		**/
		...
		private function createIntialChildren():void {
			...
			m_lblDaysNames.width = 1200;
			m_lblDaysNames.height = 30;
			m_lblDaysNames.styleName = "lblDaysNames";
			m_lblDaysNames.text = "   Domingo               Lunes                   Martes                   Miércoles               Jueves                 Viernes                 Sábado ";
			...
			**/
			...
			for(i = 0; i < arrDays.length; i++)	{
				...
				if(arrDays[i] == -1) {
					...
					objDayCell.txtDesc.toolTip = "Haga click para ver el detalle";
					...
					for(var j:int = 0; j < DataHolder.getInstance().dataProvider.length; j++) {
						...
						if(CommonUtils.compareDate(obj.fecha, objDayCell.data.date)) {
							...
							objDayCell.txtDesc.text = "Vehículos por entregar: " + obj.total;
