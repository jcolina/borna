package Componentes.Agenda.classes.model
{
	import flash.events.EventDispatcher;
	
	import mx.collections.ArrayCollection;
	
	import Componentes.Agenda.classes.events.CustomEvents;
	...
	*/
	public class DataHolder extends EventDispatcher 
	{
		...
		private var _arrDatosSemana:ArrayCollection; 
		...
		public function buscarDatos(fecha:String):Object
		{
			...
			for(var i:int = 0; i < m_arrEvents.length; i++){
				if(m_arrEvents[i].Entrega.toString() == fecha){
					encontrado = true;
				}
			}
			return encontrado;
		...
		public function getDatos(fecha:String):Object
		{
			...
			for(var i:int = 0; i < m_arrEvents.length; i++){
				if(m_arrEvents[i].Entrega == fecha){
		...
		public function set arrDatosSemana(_arrDatosSemana:ArrayCollection):void{
			this._arrDatosSemana = _arrDatosSemana;
		}
		
		public function get arrDatosSemana():ArrayCollection{
			return this._arrDatosSemana;
		}
	}
}